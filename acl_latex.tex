...
\begin{abstract}
% Modelling human variation in rating tasks is crucial for enabling AI systems for personalization, pluralistic model alignment, and computational social science.
...
We propose representing individuals using natural language {\em value profiles} -- descriptions of underlying values compressed from in-context demonstrations -- along with a steerable decoder model to estimate individual ratings based on a rater representation. To measure the predictive information in a representations, we introduce an information-theoretic methodology and find that demonstrations contain the most information, followed by value profiles and then demographics. However, value profiles effectively compress the useful information from demonstrations ($>$70\% information preservation) and offer advantages in terms of scrutability, interpretability, and steerability.
...
Going beyond test set performance, we show that the decoder models change ratings in line with semantic profile differences, are well-calibrated, and can help explain instance-level disagreement by simulating an annotator population.
...
% the target is a single "correct" answer (to the degree that people are self-consistent, \citealt{abercrombie2023consistencykeydisentanglinglabel}) instead of a distribution,
...
% \includegraphics[width=0.85\textwidth]{files/vp_roma_new.pdf}
% \small{
