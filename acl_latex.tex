...
In absence of this data, we propose to infer $V$ from observed 
% TODO - add back in
% example
ratings $E_n$ through an autoencoder setup.
% \end{itemize}
...
In practice, we initialize the encoder and decoder parameters $\phi$ and $\theta$ as prompted language models
% (prompts in App.\ref{app:additionalfigures})
(prompts in App. \ref{fig:encoderprompt}/\ref{fig:decoderprompt}).
% TODO - add back in
% (prompts in Figs. \ref{fig:encoderprompt}/\ref{fig:decoderprompt}).
...
Now, we compare decoder performance across rater representation settings
(see Figs. \ref{fig:nllresults}, \ref{fig:rater_info}).
% TODO - add back in
% (see Figs. \ref{fig:nllresults}, \ref{fig:rater_info},  \ref{fig:accresults}).
...
Across all four large datasets, providing the decoder with in-context examples of the rater's previous annotations significantly improved the prediction of their ratings on held-out test data in both accuracy and test
loss.
% TODO - add back in 
% loss, i.e.  $I_\mathcal{V}(E_n(\mathcal{R}) \to \mathcal{Y} \mid \mathcal{X}) > 0$) with $p < .001$
We observe a similar, but less significant, drop in loss/increase in accuracy on the two small datasets. This indicates that rater demonstrations offer useful information for disentangling human variation.
...
We also experiment with providing one demographic variable at a time (i.e., grouping by demographic) and providing value profiles and demographics together.
% TODO - add back in
% (cf. App. \ref{app:additionalexperiments}/Fig. \ref{fig:demographicresults}).
As expected, single demographics provide less information than including all demographics. Also, demographics and value profiles can contain complementary information, with the best performing representation generally being demographics and value profiles together.
% TODO - remove
For more details, see App. \ref{fig:demographicresults}/ \ref{app:additionalexperiments}.
...
For DICES, even two clusters were quite predictive. The corresponding value clusters relate to overall sensitivity to toxicity (e.g., profile 1: "High tolerance for offensive language"; "Narrow definition of toxicity" vs. profile 2: "Strong reaction to overt negativity, "Sensitivity to potential harm").
% Meanwhile, when going to four clusters, more nuance enters in (e.g., "Context and intent matter"). In other words, it seems that 1) overall sensitivity to toxicity is an important dimension in explaining variation, and 2) there are clusters of people who hold more nuanced views.
For OpinionQA, descriptors that have to do with politics are often used (e.g., "Economically Conservative, but Populist on Trade"). For HK, which required more clusters to effectively predict, there are many specific phrases about what kinds of things the rater may or may not find offensive.
% (e.g., "Profanity tolerance"; "Discomfort with stereotyping"; "Acceptance of strong opinions"; etc.).
Meanwhile, for PR the clusters center around potentially conflicting chatbot preferences (e.g., "Appreciates simplicity" vs. "Appreciates nuanced and comprehensive answers").
% TODO - add back in
...
% TODO - add back in
% Proper calibration allows for exciting potential applications, such as disentangling  value-related epistemic uncertanity from aleatoric uncertainty in rater variation. (See App. \ref{app:applications} for more discussion.)
...
% TODO - add back for camera ready
